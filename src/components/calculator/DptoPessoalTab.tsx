...
import {
  ...
  loadDptoPessoalColumnOrderFromStorage,
  saveDptoPessoalColumnOrderToStorage,
  clearDptoPessoalColumnOrderStorage,
...
export const DptoPessoalTab: React.FC<DptoPessoalTabProps> = ({ rows, onRowsChange, empresas }) => {
  ...
  // Ordem das colunas móveis da tabela Dpto. Pessoal
  const DEFAULT_DPTO_PESSOAL_COLUMNS: { field: SortField; label: string; numeric?: boolean }[] =
    useMemo(
      () => [
        { field: 'empresa', label: 'EMPRESAS' },
        { field: 'cnpj', label: 'CNPJ' },
        { field: 'zona', label: 'ZONA' },
        { field: 'numFunc', label: 'Nº FUNC.', numeric: true },
      ],
      [],
    )

  const [columnOrder, setColumnOrder] = useState<string[]>(() => {
    return loadDptoPessoalColumnOrderFromStorage()
  })
  ...
  const orderedColumns = useMemo(() => {
    const defaultFields = DEFAULT_DPTO_PESSOAL_COLUMNS
    ...
    return list
  }, [columnOrder, DEFAULT_DPTO_PESSOAL_COLUMNS])

  const isCustomOrderActive = useMemo(() => {
    if (columnOrder.length === 0) return false
    const defaultKeys = DEFAULT_DPTO_PESSOAL_COLUMNS.map((c) => c.field)
  ...
  const handleColumnDrop = (_e: React.DragEvent<HTMLTableCellElement>, targetKey: string) => {
    ...
    saveDptoPessoalColumnOrderToStorage(newOrder)
    setDraggedColKey(null)
    setDropTargetColKey(null)
    setDropSide(null)
                      ...
                      <tr className="bg-[#380638] text-white select-none">
                        ...
                        {orderedColumns.map((col) => (
                          <ResizableTh
                            ...
                            minWidth={MIN_DPTO_PESSOAL_COL_WIDTHS[col.field]}
                            resizable={true}
