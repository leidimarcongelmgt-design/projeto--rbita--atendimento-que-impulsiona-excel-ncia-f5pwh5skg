...
import {
  ...
  deleteDptoFiscalRecord,
  saveDptoFiscalRecord,
  loadDptoFiscalColumnOrderFromStorage,
  saveDptoFiscalColumnOrderToStorage,
  clearDptoFiscalColumnOrderStorage,
...
export const DptoFiscalTab: React.FC<DptoFiscalTabProps> = ({ rows, onRowsChange, empresas }) => {
  ...
  // Ordem das colunas móveis da tabela Dpto. Fiscal
  const DEFAULT_DPTO_FISCAL_COLUMNS: { field: SortField; label: string; numeric?: boolean }[] =
    useMemo(
      () => [
        { field: 'empresa', label: 'EMPRESAS' },
        { field: 'cnpj', label: 'CNPJ' },
        { field: 'zona', label: 'ZONA' },
        { field: 'peso', label: 'PESO', numeric: true },
      ],
      [],
    )

  const [columnOrder, setColumnOrder] = useState<string[]>(() => {
    return loadDptoFiscalColumnOrderFromStorage()
  })
  ...
  const orderedColumns = useMemo(() => {
    const defaultFields = DEFAULT_DPTO_FISCAL_COLUMNS
    ...
    return list
  }, [columnOrder, DEFAULT_DPTO_FISCAL_COLUMNS])

  const isCustomOrderActive = useMemo(() => {
    if (columnOrder.length === 0) return false
    const defaultKeys = DEFAULT_DPTO_FISCAL_COLUMNS.map((c) => c.field)
  ...
  const handleColumnDrop = (_e: React.DragEvent<HTMLTableCellElement>, targetKey: string) => {
    ...
    saveDptoFiscalColumnOrderToStorage(newOrder)
    setDraggedColKey(null)
    setDropTargetColKey(null)
    setDropSide(null)
  ...
  const applyImport = (
    ...
    saveDptoFiscalToStorage(result.rows)
  ...
  const handleSyncFromEmpresas = () => {
    ...
    onRowsChange(synced)
    saveDptoFiscalToStorage(synced)
  ...
  const handlePesoChange = (id: string, value: string) => {
    let changedRow: DptoFiscalRow | undefined
      ...
      const val =
        trimmed === ''
          ...
          : !isNaN(Number(trimmed.replace(',', '.')))
            ...
            : value
      changedRow = { ...r, peso: val }
      return changedRow
    })
    onRowsChange(updated)
    if (changedRow) saveDptoFiscalRecord(changedRow).catch(() => {})
  }
  ...
  const handleAddManualRow = (e: React.FormEvent) => {
    ...
    onRowsChange(updated)
    saveDptoFiscalToStorage(updated)
    setNewEmpresaNome('')
    setNewEmpresaCnpj('')
    setNewZona('')
    setNewPeso('')
  ...
  const handleDeleteRow = (id: string, empresaNome: string) => {
    deleteDptoFiscalRecord(id).catch(() => {})
                  ...
                  <table className="w-full text-left border-collapse text-xs table-fixed">
                    <thead>
                      <tr className="bg-[#380638] text-white select-none">
                        <ResizableTh
                          width={widths.index}
                          minWidth={MIN_DPTO_FISCAL_COL_WIDTHS.index}
                          resizable={true}
                        ...
                        {orderedColumns.map((col) => (
                          <ResizableTh
                            ...
                            minWidth={MIN_DPTO_FISCAL_COL_WIDTHS[col.field]}
                            resizable={true}
                        ...
                        <th
                          style={{
                            ...
                            minWidth: `${MIN_DPTO_FISCAL_COL_WIDTHS.acoes}px`,
                          }}
                          ...
                          <tr key={row.id} className="hover:bg-slate-50 transition-colors group">
                            ...
                            {orderedColumns.map((col) => {
                              ...
                              return (
                                <td
                                  key={col.field}
                                  className="py-2 px-3 border-r border-slate-100 text-right overflow-hidden"
                                >
                                  <Input
                                    type="text"
                                    value={row.peso}
