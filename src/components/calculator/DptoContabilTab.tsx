...
interface DptoContabilTabProps {
  rows: DptoContabilRow[]
  onRowsChange: (rows: DptoContabilRow[]) => void
  empresas: EmpresaRow[]
}
...
export const DptoContabilTab: React.FC<DptoContabilTabProps> = ({
  rows,
  onRowsChange,
  empresas,
}) => {
  ...
  const handleFileSelected = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const parsed = await parseDptoContabilFile(file)
  ...
  // Estatísticas de Contábil (preenchidas e distintas)
  const stats = useMemo(() => {
    let preenchidas = 0
    const valoresDistintos = new Set<string>()

    for (const r of rows) {
      const val = (r.contabil || '').trim()
      if (val !== '') {
        preenchidas++
        valoresDistintos.add(val.toLowerCase())
      }
    }

    return {
      preenchidas,
      distintos: valoresDistintos.size,
    }
  }, [rows])
      ...
      <div className="space-y-6 animate-fade-in">
        ...
        <Card className="border border-slate-200 card-shadow bg-white">
          ...
          <CardContent className="pt-6 space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div className="p-4 rounded-lg border border-slate-200 bg-slate-50/60 flex items-center justify-between">
                <div>
                  <span className="text-xs text-slate-500 font-medium">Contábil Preenchido</span>
                  <div className="flex items-baseline gap-2 mt-0.5">
                    <p className="text-2xl font-bold text-slate-900">{stats.preenchidas}</p>
                    {rows.length > 0 && (
                      <span className="text-xs text-slate-500">
                        de {rows.length} ({stats.distintos} distinto
                        {stats.distintos === 1 ? '' : 's'})
                      </span>
                    )}
                  </div>
                  ...
                  <table className="w-full text-left border-collapse text-xs table-fixed">
                    <thead>
                      <tr className="bg-[#380638] text-white select-none">
                        <ResizableTh
                          width={widths.index}
                          minWidth={MIN_DPTO_CONTABIL_COL_WIDTHS.index}
                          resizable={true}
                        ...
                        <th
                          style={{
                            ...
                            minWidth: `${MIN_DPTO_CONTABIL_COL_WIDTHS.acoes}px`,
                          }}
                      ...
                      <tfoot>
                        <tr className="bg-slate-100 font-semibold text-slate-800 border-t-2 border-slate-300">
                          ...
                          <td className="py-2.5 px-3 text-xs text-[#1E3A5F] font-semibold truncate">
                            {
                              filteredAndSortedRows.filter((r) => (r.contabil || '').trim() !== '')
                                .length
                            }{' '}
                            preenchida(s)
                          </td>
          ...
          <DialogContent className="sm:max-w-md">
            <DialogHeader>
              ...
              <DialogDescription className="text-xs text-slate-500 pt-1">
                Já existem {rows.length} empresa(s) cadastradas em Dpto. Contábil. A planilha contém{' '}
                {pendingFileRows?.rows.length ?? 0} registro(s) com informações da coluna Contábil.
              </DialogDescription>
            </DialogHeader>
            ...
            <div className="py-3 text-xs text-slate-600 space-y-2">
              <div className="p-3 rounded-md bg-slate-50 border border-slate-200">
                ...
                Apaga os registros atuais de Dpto. Contábil e mantém apenas os novos registros.
              </div>
              <div className="p-3 rounded-md bg-slate-50 border border-slate-200">
                <span className="font-semibold text-slate-800 block mb-0.5">
                  Adicionar / Atualizar existentes
                </span>
                Acrescenta novos registros e atualiza os dados da coluna Contábil de empresas que já
                existam na lista pelo nome ou CNPJ.
              </div>
            </div>
        ...
        <Dialog open={addModalOpen} onOpenChange={setAddModalOpen}>
          <DialogContent className="sm:max-w-md">
            <DialogHeader>
              <DialogTitle className="text-base font-bold text-slate-900">
                Adicionar Empresa em Dpto. Contábil
              </DialogTitle>
              <DialogDescription className="text-xs text-slate-500 pt-1">
                Cadastre o nome da empresa e o respectivo valor da coluna contábil.
              </DialogDescription>
            </DialogHeader>
            ...
            <form onSubmit={handleAddManualRow} className="space-y-4 pt-2">
              <div className="space-y-1.5">
                <Label
                  htmlFor="manualContabilEmpresa"
                  className="text-xs font-semibold text-slate-700"
                >
                  Nome da Empresa *
                </Label>
                <Input
                  id="manualContabilEmpresa"
              ...
              <div className="space-y-1.5">
                <Label
                  htmlFor="manualContabilCnpj"
                  className="text-xs font-semibold text-slate-700"
                >
                  CNPJ (opcional)
                </Label>
                <Input
                  id="manualContabilCnpj"
                  placeholder="00.000.000/0000-00"
              ...
              <div className="space-y-1.5">
                <Label htmlFor="manualContabilVal" className="text-xs font-semibold text-slate-700">
                  CONTÁBIL
                </Label>
                <Input
                  id="manualContabilVal"
                  type="text"
                  placeholder="Ex: Sim, Externo, Lucro Real..."
                  value={newContabil}
          ...
          <AlertDialogContent>
            <AlertDialogHeader>
              <AlertDialogTitle className="text-base font-bold text-slate-900">
                Limpar registros do Dpto. Contábil?
              </AlertDialogTitle>
              <AlertDialogDescription className="text-xs text-slate-500 pt-1">
                Essa ação removerá todas as {rows.length} empresas e seus respectivos dados
                contábeis desta aba. Essa ação não poderá ser desfeita.
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
